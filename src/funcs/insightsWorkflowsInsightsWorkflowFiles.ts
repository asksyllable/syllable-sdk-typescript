 ...
 * still show up here.
 */
export function insightsWorkflowsInsightsWorkflowFiles(
  client: SyllableSDKCore,
  request: operations.InsightsWorkflowFilesRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ListResponseWorkflowFileRow,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.InsightsWorkflowFilesRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/api/v1/insights/workflows/{workflow_id}/files")(
    pathParams,
  );
  ...
  const context = {
    ...
    operationID: "insights_workflow_files",
    oAuth2Scopes: null,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ListResponseWorkflowFileRow$inboundSchema),
