 ...
 * List Twilio phone numbers.
 */
export async function channelsTwilioNumbersList(
  client: SyllableSDKCore,
  request: operations.ChannelsTwilioNumbersListRequest,
  options?: RequestOptions,
): Promise<
  Result<
    components.TwilioListNumbersResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ChannelsTwilioNumbersListRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.TwilioListNumbersResponse$inboundSchema),
