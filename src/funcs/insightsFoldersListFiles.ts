 ...
 */
export async function insightsFoldersListFiles(
  client: SyllableSDKCore,
  request: operations.InsightsUploadListFilesRequest,
  options?: RequestOptions,
): Promise<
  Result<
    components.ListResponseInsightsUploadFile,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.InsightsUploadListFilesRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/api/v1/insights/folders/{folder_id}/files")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.ListResponseInsightsUploadFile$inboundSchema),
