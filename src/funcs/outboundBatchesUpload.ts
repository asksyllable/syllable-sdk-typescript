...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.OutboundBatchUploadRequest$outboundSchema.parse(value),
  ...
  if (payload.Body_outbound_batch_upload != null) {
    if (payload.Body_outbound_batch_upload.file !== undefined) {
      if (isBlobLike(payload.Body_outbound_batch_upload.file)) {
        ...
        payload.Body_outbound_batch_upload.file.content instanceof Uint8Array
      ) {
        const contentType =
          getContentTypeFromFileName(
            payload.Body_outbound_batch_upload.file.fileName,
        ...
        appendForm(
          ...
          "file",
          new Blob([
            new Uint8Array(payload.Body_outbound_batch_upload.file.content)
              .buffer,
          ], { type: contentType }),
          payload.Body_outbound_batch_upload.file.fileName,
        );
  ...
  const path = pathToFunc("/api/v1/outbound/batches/{batch_id}/upload_batch")(
    pathParams,
  );
