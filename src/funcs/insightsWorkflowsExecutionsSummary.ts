 ...
 */
export function insightsWorkflowsExecutionsSummary(
  client: SyllableSDKCore,
  request: operations.InsightsWorkflowExecutionsSummaryRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.InsightWorkflowExecutionSummary,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.InsightsWorkflowExecutionsSummaryRequest$outboundSchema.parse(
        value,
      ),
  ...
  const path = pathToFunc(
    "/api/v1/insights/workflows/{workflow_id}/executions/summary",
  )(pathParams);
  ...
  const context = {
    ...
    operationID: "insights_workflow_executions_summary",
    oAuth2Scopes: null,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.InsightWorkflowExecutionSummary$inboundSchema),
