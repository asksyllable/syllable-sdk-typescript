...
* A **tool** is a function that an agent can use to carry out actions like calling APIs or looking up information, as part of following the instructions defined in the prompt. We'll need two tools: one to give the weather agent the ability to query the [Open-Meteo API](https://open-meteo.com/en/docs) for real-time weather data, and one to give it access to our data source with the general weather information. We'll be linking these tools to the prompt, so that any agents using the prompt will be able to access them.
