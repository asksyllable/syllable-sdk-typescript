...
If you have configured a static parameter on a tool (see "Static parameters" section of [Tools](/mintlify/Resources/Tools)), linked the tool to a prompt, and then linked that prompt to one or more agents, you can set override values for the static parameters at the agent level. When the agent calls the tool, for each static parameter, it will use its own override value if one exists; if not, it will use the tool's default value for that static parameter if one exists. If there is no override value or default value for a given static parameter, the tool call will use a `null `value for that parameter.
