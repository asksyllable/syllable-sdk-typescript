...
2. Create a [tool](/mintlify/Resources/tools) with the following schema (specify whatever tool name you want).
...
Note that if you later create another agent that you want to have access to different data sources, you can link it to the same prompt but override the default "doc" values on that agent (see ["Tool configuration"](/mintlify/workspaces/ToolConfig)).
...
4. In the agent's ["Session Configuration" section](/mintlify/workspaces/SessionConfig.mdx), for the variable `vars.domain`, enter the value `doc://yourdatasourcenamehere`.