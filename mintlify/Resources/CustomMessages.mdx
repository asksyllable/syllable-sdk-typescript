...
Custom messages are configurable text that the agent reads to the user - for example, every agent must be associated with a custom message to be delivered as a greeting at the beginning of the conversation. Custom messages are set up with time-based logic so that the agent can deliver different variations on the message depending on the date and time of the conversation.

## Configure a Custom Message

![](/images/create_custom_message.jpg)

* **Message name:** The name of your Custom Message. Use a name that’s easily remembered and specific enough to distinguish it from other Custom Messages.

* **Label (optional):** A plain text indicator to help you remember your Custom Message. It should be different from the name.
