...
Messages are configurable text that the agent reads to the user. Every agent must be associated with a message to be delivered as a greeting at the beginning of the conversation. Messages are set up with time-based logic so that the agent can deliver different variations on the message depending on the date and time of the conversation.
...
* **Language menu tag button**: This button allows you to insert a language menu tag into the message (see [language group](/mintlify/Resources/LanguageGroups.mdx)s)
